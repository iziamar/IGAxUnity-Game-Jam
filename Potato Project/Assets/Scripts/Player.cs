...
public class Player : MonoBehaviour
{
    ...
    public TextMeshProUGUI fertUI;
    public float totalPotatoesFed;
    ...
    public bool isGrounded()
    {
        ...
        if (canCheckGround && Physics2D.BoxCast(transform.position - transform.up * castDistance, boxSize, 0f, Vector2.down, 0f, groundMask))
        {
            Debug.Log("Grounded");
            if (rb.velocity.y < 0)
                rb.velocity = new Vector2(rb.velocity.x, 0f);
